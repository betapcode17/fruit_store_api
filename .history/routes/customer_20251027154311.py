...
@router.get("/viewCustomer/{bill_id}", response_model=CustomerResponse)
def view_bill(cus_id: int, db: Session = Depends(get_db)):
    bill = db.query(Customer).filter(Bill.bill_id == bill_id).first()
    if not bill:
